...
def generate_signals(df, symbol):
    ...
    if latest_row['rsi'] < SCALPING_SETTINGS['rsi_oversold'] and prev_row['rsi'] < latest_row['rsi']:
        buy_reasons.append(f"RSI در ناحیه oversold ({latest_row['rsi']:.2f})")
    
    if (prev_row['ema_short'] <= prev_row['ema_medium'] and 
        latest_row['ema_short'] > latest_row['ema_medium']):
        buy_reasons.append("کراس EMA کوتاه‌مدت به بالای EMA میان‌مدت")
    
    if latest_row['close'] <= latest_row['bb_lower'] * 1.01:
        buy_reasons.append("قیمت نزدیک/زیر باند پایین بولینگر")
    
    # استراتژی فروش
    sell_reasons = []
    if latest_row['rsi'] > SCALPING_SETTINGS['rsi_overbought'] and prev_row['rsi'] > latest_row['rsi']:
        sell_reasons.append(f"RSI در ناحیه overbought ({latest_row['rsi']:.2f})")
    
    if (prev_row['ema_short'] >= prev_row['ema_medium'] and 
        latest_row['ema_short'] < latest_row['ema_medium']):
        sell_reasons.append("کراس EMA کوتاه‌مدت به پایین EMA میان‌مدت")
    
    if latest_row['close'] >= latest_row['bb_upper'] * 0.99:
        sell_reasons.append("قیمت نزدیک/بالای باند بالایی بولینگر")
